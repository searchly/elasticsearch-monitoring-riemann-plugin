...
public class NodeStatsRiemannEvent {
    ...
    private void currentIndexingRate(NodeIndicesStats nodeIndicesStats) {
        ...
        riemannClient.event().host(hostDefinition).
                service("Current Indexing Rate").description("current_indexing_rate").tags(tags).attributes(attributes).state(RiemannUtils.getState(indexingCurrent, 300, 1000)).metric(indexingCurrent).send();
    }
    ...
    private void heapRatio(JvmStats jvmStats) {
        ...
        riemannClient.event().host(hostDefinition).
                service("Heap Usage Ratio %").description("heap_usage_ratio").tags(tags).attributes(attributes).state(RiemannUtils.getState(heapRatio, 85, 95)).metric(heapRatio).send();
    }
    ...
    private void currentQueryRate(NodeIndicesStats nodeIndicesStats) {
        ...
        riemannClient.event().host(hostDefinition).
                service("Current Query Rate").description("current_query_rate").tags(tags).attributes(attributes).state(RiemannUtils.getState(queryCurrent, 50, 70)).metric(queryCurrent).send();
    }
    ...
    private void currentFetchRate(NodeIndicesStats nodeIndicesStats) {
        ...
        riemannClient.event().host(hostDefinition).
                service("Current Fetch Rate").description("current_fetch_rate").tags(tags).attributes(attributes).state(RiemannUtils.getState(fetchCurrent, 50, 70)).metric(fetchCurrent).send();
    }

    private void totalThreadCount(JvmStats jvmStats) {
        int threadCount = jvmStats.getThreads().getCount();
        riemannClient.event().host(hostDefinition).
                service("Total Thread Count").description("total_thread_count").tags(tags).attributes(attributes).state(RiemannUtils.getState(threadCount, 150, 200)).metric(threadCount).send();
    }

    private void systemLoadOne(OsStats osStats) {
        double[] systemLoad = osStats.getLoadAverage();
        riemannClient.event().host(hostDefinition).
                service("System Load(1m)").description("system_load").tags(tags).attributes(attributes).state(RiemannUtils.getState((long) systemLoad[0], 2, 5)).metric(systemLoad[0]).send();
    }

    private void systemLoadFive(OsStats osStats) {
        double[] systemLoad = osStats.getLoadAverage();
        riemannClient.event().host(hostDefinition).
                service("System Load(5m)").description("system_load").tags(tags).attributes(attributes).state(RiemannUtils.getState((long) systemLoad[1], 2, 5)).metric(systemLoad[1]).send();
    }

    private void systemLoadFifteen(OsStats osStats) {
        double[] systemLoad = osStats.getLoadAverage();
        riemannClient.event().host(hostDefinition).
                service("System Load(15m)").description("system_load").tags(tags).attributes(attributes).state(RiemannUtils.getState((long) systemLoad[2], 2, 5)).metric(systemLoad[2]).send();
    }

    private void systemMemory(OsStats osStats) {
        short memoryUsedPercentage = osStats.getMem().getUsedPercent();
        riemannClient.event().host(hostDefinition).
                service("System Memory Usage %").description("system_memory_usage").tags(tags).attributes(attributes).state(RiemannUtils.getState(memoryUsedPercentage, 80, 90)).metric(memoryUsedPercentage).send();
    ...
    private void systemFile(FsStats fsStats) {
        for (FsStats.Info info : fsStats) {
            ...
            riemannClient.event().host(hostDefinition).
                    service("Disk Usage %").description("system_disk_usage").tags(tags).attributes(attributes).state(RiemannUtils.getState(usageRatio, 80, 90)).metric(usageRatio).send();
        }
    }
}
